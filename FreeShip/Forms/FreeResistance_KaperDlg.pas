...
begin
   ...
   if FFreeship.ProjectSettings.ProjectUnits=fuImperial then Chart.LeftAxis.Title.Caption:=Userstring(272)+' [lbs]'
                                                        else Chart.LeftAxis.Title.Caption:=Userstring(272)+' [N]';
   ResultsMemo.Clear;
   ...
   FFreeship.CreateOutputHeader(Userstring(274)+', '+Userstring(275)+'.',ResultsMemo.Lines);
   ResultsMemo.Lines.Add('----------------------------------------------------------------------------------');
   ResultsMemo.Lines.Add('');
   ResultsMemo.Lines.Add(Userstring(250));
   ResultsMemo.Lines.Add('');
   ResultsMemo.Lines.Add(Userstring(256));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(276),30)+' : '+FloatToStrF(Ewl,ffFixed,6,3)+#32+LengthStr(FFreeship.ProjectSettings.ProjectUnits));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(18),30)+' : '+FloatToStrF(Bwl,ffFixed,6,3)+#32+LengthStr(FFreeship.ProjectSettings.ProjectUnits));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(257),30)+' : '+FloatToStrF(Draft,ffFixed,6,3)+#32+LengthStr(FFreeship.ProjectSettings.ProjectUnits));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(10),30)+' : '+FloatToStrF(Ws,ffFixed,6,2)+#32+AreaStr(FFreeship.ProjectSettings.ProjectUnits));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(8),30)+' : '+FloatToStrF(Cp,ffFixed,6,4));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(4),30)+' : '+FloatToStrF(Displ,ffFixed,6,4)+#32+WeightStr(FFreeship.ProjectSettings.ProjectUnits));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(277),30)+' : '+FloatToStrF(Ie,ffFixed,6,3)+' [degr.]');
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(11),30)+' : '+FloatToStrF(LCB,ffFixed,6,3));
   ResultsMemo.Lines.Add(Space(6)+MakeLength(Userstring(278),30)+' : '+FloatToStrF(At_Ax,ffFixed,6,4));
   ResultsMemo.Lines.Add('');
   ResultsMemo.Lines.Add('----------------------------------------------------------------------------------');
   ResultsMemo.Lines.Add('');
   ResultsMemo.Lines.Add('');
   ...
   begin
      ResultsMemo.Lines.Add(Userstring(271)+' of 0.48 .. 0.64');
      ValidData:=False;
   end;
   if (At_Ax<0.0) or (At_Ax>0.04) then
   begin
      ResultsMemo.Lines.Add(Userstring(278)+' of 0.00 .. 0.04');
      ValidData:=False;
   end;
   ...
   ResultsMemo.Lines.Add('');
   ResultsMemo.Lines.Add('   +-------+-------+-------+---------+---------+---------+---------+');
   ResultsMemo.Lines.Add('   | Speed | Speed |  S/L  | R frict.| R resid.| R total | Spilman |');
   if FFreeship.ProjectSettings.ProjectUnits=fuImperial then ResultsMemo.Lines.Add('   |  [kn] | [ft/s]| ratio |  [lbs]  |  [lbs]  |  [lbs]  |  [lbs]  |')
                                                        else ResultsMemo.Lines.Add('   |  [kn] | [m/s] | ratio |   [N]   |   [N]   |   [N]   |   [N]   |');
   ResultsMemo.Lines.Add('   +-------+-------+-------+---------+---------+---------+---------+');
   ...
   begin
      ...
      begin
         if FFreeship.ProjectSettings.ProjectUnits=fuImperial then
         begin
            ResultsMemo.Lines.Add('   | '+Makelength(Speed,2,5)+' | '+        // speed in knots
            ...
            Chart.Series[0].AddXY(Speed,Rfrictional);
            Chart.Series[1].AddXY(Speed,Rresidual);
            Chart.Series[2].AddXY(Speed,Rtotal);
            Chart.Series[3].AddXY(Speed,RSpilman);
         end else
         begin
            ResultsMemo.Lines.Add('   | '+Makelength(Speed,2,5)+' | '+               // speed in knots
                                  Makelength(Speed*1852/3600,2,5)+' | '+             // speed in m/sec
                                  Makelength(SpeedLengthRatio,3,5)+' | '+            // speed/length ratio
                                  Makelength(PoundsToNewton(RFrictional),3,7)+' | '+ // Frictional resistance
                                  Makelength(PoundsToNewton(RResidual),3,7)+' | '+   // Residual resistance
                                  Makelength(PoundsToNewton(RTotal),3,7)+' | '+      // Total resistance
                                  Makelength(PoundsToNewton(RSpilman),3,7)+' |');    // Resistance according to Spilman formula
            Chart.Series[0].AddXY(Speed,PoundsToNewton(Rfrictional));
            Chart.Series[1].AddXY(Speed,PoundsToNewton(Rresidual));
            Chart.Series[2].AddXY(Speed,PoundsToNewton(Rtotal));
            Chart.Series[3].AddXY(Speed,PoundsToNewton(RSpilman));
         end;
      end;
      Speed:=Speed+0.1;
...
begin
   ...
   Chart.LeftAxis.Title.Caption:=Userstring(272)+' [lbs]';
