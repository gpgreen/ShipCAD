...
namespace Geometry {
  ...
  public:
    ...
    void assign(const Spline& spline);
    qreal coord_length(qreal t1, qreal t2);
    void chord_length_approximation(qreal percentage);
    virtual void clear();
    virtual void create();
...
begin
   if Units=fuImperial then result:='[ft2]'
                       else result:='[m2]';
end;{AreaStr}
...
begin
   if Units=fuImperial then result:='[ft]'
                       else result:='[m]';
end;{LengthStr}
...
begin
   if Units=fuImperial then result:='[ft4]'
                       else result:='[m4]';
end;{InertiaStr}

function VolStr(Units:TFreeUnitType):String;
begin
   if Units=fuImperial then result:='[ft3]'
                       else result:='[m3]';
end;{VolStr}
...
begin
   if Units=fuImperial then result:='[lbs/ft3]'
                       else result:='[t/m3]';
end;{DensityStr}

function WeightStr(Units:TFreeUnitType):String;
begin
   if Units=fuImperial then result:='[tons]'
                       else result:='[tonnes]';
end;{WeightStr}
...
begin
   ...
   if abs(Len)>1e-6 then Result:=Sqrt(N.x*N.x+N.y*N.y+N.z*N.z)/Len
...
begin
   ...
   begin
      ...
      begin
         ...
         if (Pt.X>=0) and (Pt.Y>=0) and (Pt.X<=Screen.Width) and (Pt.Y<=Screen.Height) then Result:=Sqrt(Sqr(Pt.X-X)+Sqr(Pt.Y-Y))
                                                                                       else Result:=1e10;
...
begin
   ...
   begin
      ...
      try
         ...
         Resolution:=GetDeviceCaps(Printer.Handle, LogPixelsX);
...
var I,J,K,N       : Integer;
    ...
    begin
       Result:=sqrt(sqr(extended(P2.X-P1.X))+sqr(extended(P2.Y-P1.Y)));
    end;{Distance2D}

    function Distance3D(P1,P2:T3DCoordinate):Extended;
    begin
       Result:=sqrt(sqr(extended(P2.X-P1.X))+sqr(extended(P2.Y-P1.Y))+sqr(extended(P2.Z-P1.Z)));
    end;{Distance3D}
   ...
   begin
      Result:=0.5*(extended(P1.x-P2.x)*extended(P1.y+P2.y)+extended(P2.x-P3.x)*extended(P2.y+P3.y)+extended(P3.x-P1.x)*extended(P3.y+P1.y));
   end;{TriangleArea}
...
begin
   ...
   begin
      ...
      Result.X:=A*FPoints[Lo].X+B*FPoints[Hi].X+((A*A*A-A)*FDerivatives[Lo].X+(B*B*B-B)*(Fderivatives[Hi].X))*(H*H)/6;
      Result.Y:=A*FPoints[Lo].Y+B*FPoints[Hi].Y+((A*A*A-A)*FDerivatives[Lo].Y+(B*B*B-B)*(Fderivatives[Hi].Y))*(H*H)/6;
      Result.Z:=A*FPoints[Lo].Z+B*FPoints[Hi].Z+((A*A*A-A)*FDerivatives[Lo].Z+(B*B*B-B)*(Fderivatives[Hi].Z))*(H*H)/6;
   end;
...
begin
   ...
   begin
      ...
      begin
         ...
         begin
            if Alpha[X]=0.0 then NewPoints[J-1][X]:=FControlpoints[J-1][X-1] else
               if Alpha[X]=1.0 then NewPoints[J-1][X]:=FControlpoints[J-1][X] else
               begin
                  NewPoints[J-1][X].X:=(1-Alpha[X])*FControlpoints[J-1][X-1].X+alpha[X]*FControlpoints[J-1][X].X;
                  NewPoints[J-1][X].Y:=(1-Alpha[X])*FControlpoints[J-1][X-1].Y+alpha[X]*FControlpoints[J-1][X].Y;
                  NewPoints[J-1][X].Z:=(1-Alpha[X])*FControlpoints[J-1][X-1].Z+alpha[X]*FControlpoints[J-1][X].Z;
               end;
         end;
      end;
      for I:=1 to Rowcount do
      begin
         ...
         Move(Newpoints[I-1][0],FControlpoints[I-1][0],(FColcount+1)*SizeOf(T3DCoordinate));
      end;
      ...
      begin
         ...
         begin
            if Alpha[X]=0.0 then NewPoints[X][J-1]:=FControlpoints[X-1][J-1] else
               if Alpha[X]=1.0 then NewPoints[X][J-1]:=FControlpoints[X][J-1] else
               begin
                  NewPoints[X][J-1].X:=(1-Alpha[X])*FControlpoints[X-1][J-1].X+alpha[X]*FControlpoints[X][J-1].X;
                  NewPoints[X][J-1].Y:=(1-Alpha[X])*FControlpoints[X-1][J-1].Y+alpha[X]*FControlpoints[X][J-1].Y;
                  NewPoints[X][J-1].Z:=(1-Alpha[X])*FControlpoints[X-1][J-1].Z+alpha[X]*FControlpoints[X][J-1].Z;
               end;
         end;
      end;
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  P3D2.X:=P3D.X-C*2*FCurve.CurvatureScale*Normal.X;
                  P3D2.Y:=P3D.Y-C*2*FCurve.CurvatureScale*Normal.Y;
                  P3D2.Z:=P3D.Z-C*2*FCurve.CurvatureScale*Normal.Z;
                  PArray2[J-1]:=Viewport.Project(P3D2);
               end;
               Viewport.SetPenWidth(1);
...
begin
   ...
   While I<=FControlPoints.Count do
   begin
      if ((FControlPoints[I-2]=P1) and (FControlPoints[I-1]=P2)) or
         ((FControlPoints[I-1]=P1) and (FControlPoints[I-2]=P2)) then
      begin
         FControlPoints.Insert(I-1,New);
      end;
      inc(I);
   end;
...
begin
   ...
   begin
      if ((FSubdividedPoints[I-2]=P1) and (FSubdividedPoints[I-1]=P2)) or
         ((FSubdividedPoints[I-1]=P1) and (FSubdividedPoints[I-2]=P2)) then
      begin
         FSubdividedPoints.Insert(I-1,New);
      end;
      inc(I);
   end;
...
begin
   ...
   begin
      ...
      Result.X:=Result.X/(n*(N+5));
      Result.Y:=Result.Y/(n*(N+5));
      Result.Z:=Result.Z/(n*(N+5));
  ...
  begin
      ...
      begin
         Result.X:=(1/6)*P30.Coordinate.X+(2/3)*FCoordinate.X+(1/6)*P33.Coordinate.X;
         Result.Y:=(1/6)*P30.Coordinate.Y+(2/3)*FCoordinate.Y+(1/6)*P33.Coordinate.Y;
         Result.Z:=(1/6)*P30.Coordinate.Z+(2/3)*FCoordinate.Z+(1/6)*P33.Coordinate.Z;
      end else
      begin
...
var Point   : T3DCoordinate;
begin
   Point.X:=0.5*(FStartpoint.FCoordinate.X+FEndpoint.FCoordinate.X);
   Point.Y:=0.5*(FStartpoint.FCoordinate.Y+FEndpoint.FCoordinate.Y);
   Point.Z:=0.5*(FStartpoint.FCoordinate.Z+FEndpoint.FCoordinate.Z);
...
begin
   ...
   try
      ...
      begin
         ...
         Point2.Coordinate:=AddPoint(Point1.Coordinate,Direction);
